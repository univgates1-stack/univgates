...
import { Loader2, Eye, EyeOff, GraduationCap, Facebook } from 'lucide-react';
...
const StudentRegistration = ({ onSuccess }: StudentRegistrationProps) => {
  ...
  const validatePassword = (password: string) => {
    ...
    return requirements.filter((rule) => !rule.valid).map((rule) => rule.message);
  };
    ...
    <Card className="w-full max-w-md mx-auto">
      ...
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <input type="hidden" value={formData.interestedProgram ?? ''} readOnly />
          <input type="hidden" value={formData.preferredCountry ?? ''} readOnly />
